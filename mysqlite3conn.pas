...
uses
 classes, db, bufdataset, sqldb, sqlite3dyn, types, SysUtils,
  SQLiteTable3;
...
type
  ...
  { TSQLite3Connection }
  TsostSynchronous=(syncOFF,syncON,syncFULL);
  TsostJournal_mode=(jmodeDelete, jmodeTruncate, JmodePersist, jmodeMemory, jmodeWal, jmodeOff);
  TsostAuto_vacuum=(avacuumOff, avacuumIncremental, avacuumFull);
  TsostForeignKey=(fkeyOff, fkeyOn);

  TSQLiteTable2=class(TSQLiteTable)
  end;
  ...
  private
    FAuto_vacuum: TsostAuto_vacuum;
    FFkey: TsostForeignKey;
    fhandle: psqlite3;
    FJmode: TsostJournal_mode;
    FSQLiteDatabase: TSQLiteDatabase;
    FSync: TsostSynchronous;
    function GetSQLiteDatabase: TSQLiteDatabase;
    procedure SetAuto_vacuum(AValue: TsostAuto_vacuum);
    procedure SetForeignKey(AValue: TsostForeignKey);
    procedure SetJournal_mode(AValue: TsostJournal_mode);
    procedure SetSynchronised(AValue: TsostSynchronous);
  protected
  ...
  public
    ...
    procedure createtableinmemory(nametb:string);
    ...
    function GetTable(const SQL: Ansistring): TSQLiteTable2; overload;
    procedure LoadExtension(LibraryFile: string);
    property Auto_Vacuum:TsostAuto_vacuum read FAuto_vacuum write SetAuto_vacuum;
    property Journal_mode: TsostJournal_mode read FJmode write SetJournal_mode;
    property Synchronised: TsostSynchronous read FSync write SetSynchronised;
    property ForeignKey: TsostForeignKey read FFkey write SetForeignKey;
...
begin
  ...
  //FSQLiteDatabase:=TSQLiteDatabase.Create();
  {FSync:=syncfull;
  FFkey:=fkeyOff;
  FAuto_vacuum:=avacuumOff;
  FJmode:=jmodeDelete; }
end;
...
procedure TSQLite3Connection.SetAuto_vacuum(AValue: TsostAuto_vacuum);
begin
  //if FAuto_vacuum=AValue then Exit;
  FAuto_vacuum:=AValue;
  case FAuto_vacuum of
    avacuumOff:execsql('PRAGMA auto_vacuum = OFF');
    avacuumIncremental:execsql('PRAGMA auto_vacuum = INCREMENTAL');
    avacuumFull:execsql('PRAGMA auto_vacuum = FULL');
  end;
end;

function TSQLite3Connection.GetSQLiteDatabase: TSQLiteDatabase;
begin
  if Assigned(fhandle) then
  result:=FSQLiteDatabase else result:=nil;
end;

procedure TSQLite3Connection.SetForeignKey(AValue: TsostForeignKey);
begin
  //if FFkey=AValue then Exit;
  FFkey:=AValue;
  case FFkey of
    fkeyOff:execsql('PRAGMA foreign_keys = OFF');
    fkeyOn:execsql('PRAGMA foreign_keys = ON');
  end;
end;

procedure TSQLite3Connection.SetJournal_mode(AValue: TsostJournal_mode);
begin
  //if FJmode=AValue then Exit;
  FJmode:=AValue;
  case FJmode of
    jmodeoff:execsql('PRAGMA journal_mode = OFF');
    jmodeDelete:execsql('PRAGMA journal_mode = DELETE');
    jmodeTruncate:execsql('PRAGMA journal_mode = TRUNCATE');
    jmodePersist:execsql('PRAGMA journal_mode = PERSIST');
    jmodeMemory:execsql('PRAGMA journal_mode = MEMORY');
    jmodeWal:execsql('PRAGMA journal_mode = WAL');
  end;
end;

{procedure TSQLite3Connection.SetSynchronised(AValue: boolean);
begin
  if AValue <> fSync then
    begin
      if AValue then
...
procedure TSQLite3Connection.SetSynchronised(AValue: TsostSynchronous);
begin
  //if FSync=AValue then Exit;
  FSync:=AValue;
  case FSync of
    syncoff:execsql('PRAGMA synchronous = OFF');
    syncon:execsql('PRAGMA synchronous = ON');
    syncfull:execsql('PRAGMA synchronous = FULL')
  end;
end;
...
begin
  ...
  if Params.IndexOfName('journal_mode') <> -1 then
    execsql('PRAGMA journal_mode =  '+Params.Values['journal_mode']);
  if Params.IndexOfName('synchronous') <> -1 then
    execsql('PRAGMA synchronous =  '+Params.Values['synchronous']);
  if Params.IndexOfName('auto_vacuum') <> -1 then
    execsql('PRAGMA auto_vacuum =  '+Params.Values['auto_vacuum']);
  if Params.IndexOfName('temp_store') <> -1 then
    execsql('PRAGMA temp_store =  '+Params.Values['temp_store']);
  //Если база открылась, то создадим экземпляр класса TSQLiteDatabase
  if Assigned(fhandle) then
  begin
       FSQLiteDatabase:=TSQLiteDatabase.Create(fhandle);
  end;
end;
...
procedure TSQLite3Connection.createtableinmemory(nametb: string);
begin
 execsql('ATTACH ":memory:" AS '+nametb);
end;
...
function TSQLite3Connection.GetTable(const SQL: Ansistring): TSQLiteTable2;
begin
  //Result := TSQLiteTable2(SQLiteDatabase.GetTable(SQL));
  Result:=TSQLiteTable2.Create(fhandle,SQL);
end;
